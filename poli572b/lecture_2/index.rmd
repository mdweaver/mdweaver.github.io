...
* Random assignment allows us to estimate unobserved potential outcomes
...
1. Sample Difference in Means in **unbiased**
    ...
    - **none** of the samples estimate population $ACE$ exactly.
...
1. We ignored sampling **without replacement** from **finite** population (**inflates variance**)
...
- **exactly correct** (if $\tau_i$ is the same for all $i$.)
...
``` {r}
ts = randomizations[sample(1:20, 1),]
