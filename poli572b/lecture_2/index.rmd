...
## For each randomization, calculate the ACE
...
hist(treatment_groups - control_groups,
     xlab = "ACE",
     main = "Sampling Distribution of ACE",
     breaks = 20)
...
- mean of the **sampling distribution** of **sample** ACE is same as **population** ACE
- **but** none of the samples have exact population ACE. 

#### Histogram is the **exact sampling distribution** of the ACE in this experiment

- tells us how likely it is we observe sample ACE by chance (using this randomization scheme)

#### We never observe this histogram

>- we need a way to **estimate** the sampling distribution of ACE.

# Estimating the variance of the sample ACE

## Three approaches

1) Analytic/Asymptotic approach

2) Bootstrap apparoch
...
### First: we want to get variance of ACE
...
- $Y^T = \frac{1}{n}\sum\limits_{i=1}^{m}Y_i^1$
- $Y^C = \frac{1}{m}\sum\limits_{i=m+1}^{n}Y_i^0$
...
$$\widehat{\sigma^2} = \frac{1}{m-1}\sum\limits_{i=1}^{m}(Y_i^1 - Y^T)^2$$
...
Using this approach we can estimate $Var(Y^T)$ and $Var(Y^C)$. We still need $Cov(Y^T,Y^C)$ to get variance of sampling distribution of sample ACE.
...
3. i.e., variances we obtain with $\widehat{Var(ACE)} = Var(Y^T) + Var(Y^C)$ are going to be exactly correct or TOO LARGE (conservative)

## Analytic Approach

So far we've been trying to estimate the **variance** of the ACE. But variance is not directly practical: units are squared. The **standard error** or the **standard deviation of the sampling distribution** of the **ACE** is more helpful. It is the square-root of the **variance**.
...
hist(bs_ace, xlab = "Bootstrapped ACE", main = "Bootstrapped Sampling Distribution\nof ACE")
