...
$$\begin{equation}\begin{split}ACE &= \{E[Y_i(1)|Z_i = 1]\pi_1 + \overbrace{\color{red}{E[Y_i(1)|Z_i = 0]}}^{\text{plug in max/min values of Y}}\pi_0\} \\ & \phantom{=}\ - \{\underbrace{\color{red}{E[Y_i(0)|Z_i = 1]}}_{\text{plug in min/max values of Y}}\pi_1 + E[Y_i(0)|Z_i = 0]\pi_0\} \end{split} \end{equation}$$
...
For us to get an **unbiased** estimator of the average causal effect, need to make **assumptions** to permit "plugging in" observed data for missing ($\color{red}{\text{counterfactuals}}$):
...
- ... random assignment **does not guarantee** that any **particular** randomization gives us exact estimate of unobserved potential outcomes, only that this **procedure** gives us correct answer **on average**
...
In your own words, as best you understand it...

- what is a hypothesis test?
- what is a $p$ value? What does it mean?
...
**Error probabilities**: if we use this **estimator** to estimate the ACE (as a test of causal theory):

- (Type I/false positive rate) how likely are we to wrongly conclude there is a **causal effect** when in fact there is **not**? 
- (Type II/false negative rate) how likely are we to wrongly conclude there is **no causal effect** when in fact there **is**? 

What are the range of causal effect sizes we can rule out, with known error probabilities? 
...
What are the other (*counterfactual*) outcomes that we could have observed through implementing this experimental procedure/test?
...
- pair a null hypothesis (claim is false) and some alternative hypothesis (claim is true) about the estimand (complements: if one is true the other must be false, so cover the entire set of possible values of the **estimand**)
...
- Often, we use a $t$ statistic when working with means. Other comparisons may require different test statistics. (these just **standardize** the sampling distribution)
...
We want to either reject the null hypothesis as False or not reject the null hypothesis (*falsification*).
...
- Type I error: false positive. We incorrectly **reject the null** $\to$ $p$ values
- Type II error: false negative. We incorrectly **fail to reject the null** $\to$ statistical power
...
- choice of $0.05$ is arbitrary. In particle physics they use $0.0000006$
...
- only indicates probability of incorrectly rejecting null **due to random procedure**
...
Our procedure rejects that the true $ACE$ is outside of confidence interval, with an error rate of $\alpha$. Our choice of $\alpha$ indicates extent to which we can reject values of $ACE$ with severity.
...
- To prove unbiasedness of the **estimator** $\widehat{ACE}$
...
But we never observe this histogram (sampling distribution is $\color{red}{\text{counterfactual}}$)
...
Normal Distribution requires we specify:

- mean under the null hypothesis (often $0$)
- the standard deviation (which we derive from the variance)

$t$ Distribution additionally requires:

- degrees of freedom (adjustment for **estimating** the standard deviation)

---

**Thus**: we want to get variance of $\widehat{ACE}$
