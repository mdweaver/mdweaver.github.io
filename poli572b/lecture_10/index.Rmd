...
- If you cannot measure it:
    - Create a DAG for their story: Is this variable already blocked?
...
- Can you **benchmark** how "important" this confounder would have to be to explain away the estimated effect you've found?
- Can you argue that it is unlikely such a "large" unknown confounder exists?

---

### Solutions

- **Adding a variable**: you already can do this
- **Drawing a DAG**: you already can do this
- **Direction of bias**: requires we understand **omitted variable bias**.
- **Size of unknown confounder**:  leads to **sensitivity analyses**.
...
geom_dag_edges() +
geom_dag_text(aes(label = label), color ='black', parse = T) +
theme_dag()
```
...
Income is not randomly assigned; we can condition on the following:
...
Anything MISSING from this DAG?
...
But the intuition is similar. If there is an unknown confounder $U$, the bias induced is a function of the relationship between $Y \sim f(U)$ and $D \sim  f(U)$, just like in omitted variable bias.
...
(1) Proportion of Variance "explained" (predicted):

We have a model where we predict $y$ with $\hat{y}$. 

$Var_{total} = \frac{1}{n}\sum\limits_{i=1}^n (y_i - \bar{y})^2$

$Var_{residual} = \frac{1}{n}\sum\limits_{i=1}^n (y_i - \hat{y}_i)^2$

$R^2 = 1 - \frac{Var_{residual}}{Var_{total}}$

As fraction $\to 0$, residuals are smaller, $\hat{y}$ is closer to $y$.

- if we just guess $\bar{y}$ as $\hat{y}$, we get $R^2$ of 0
...
(2) Cosine similarity of vector $y$ and $\hat{y}$:

- vectors are in $n$ dimensional space
- form a right triangle when we include residuals $e$
- what is the *angle* $\theta$ between $y$ and $\hat{y}$?
- $cos(\theta)$ is $\frac{length(adjacent)}{length(hypotenuse)}$
...
proj_scalar <- sum(y * y_hat) / sum(y_hat * y_hat)
y_proj <- proj_scalar * y_hat
y_hat = y_proj
# 2. Calculate oriented angles for start and end points
# atan2(y, x) is preferred for oriented angles
angle_y <- atan2(y[2], y[1])
angle_yhat <- atan2(y_hat[2], y_hat[1])

# Calculate midpoint for theta label placement
mid_angle <- (angle_y + angle_yhat) / 2
label_r <- 1.3 # Slightly further out than the arc radius

ggplot() +
  # Draw vectors y and y_hat
  geom_textsegment(aes(x = 0, y = 0, xend = y[1], yend = y[2], label = "hypotenuse"), 
               arrow = arrow(length = unit(0.3, "cm")), color = "blue", size = 4) +
  geom_textsegment(aes(x = 0, y = 0, xend = y_hat[1], yend = y_hat[2], label = "adjacent"), 
               arrow = arrow(length = unit(0.3, "cm")), color = "red", size = 4) +
  geom_segment(aes(x = y_proj[1], y = y_proj[2], xend = y[1], yend = y[2]), 
               arrow = arrow(length = unit(0.3, "cm")), color = "green", size = 1) +
  # Draw the arc between them (Radius = 1)
  geom_arc(aes(x0 = 0, y0 = 0, r = 1, 
               start = angle_y + .45, 
               end = angle_yhat + .45), 
           color = "black", size = 0.8) +
  
  # Add labels using parse = TRUE for mathematical notation
  annotate("text", x = y[1]*.9, y = y[2]*.9, label = "y", vjust = 1, hjust = -1, color = "blue") +
  annotate("text", x = y_hat[1], y = y_hat[2], label = "hat(y)", 
           vjust = 1.5, color = "red", parse = TRUE) +
  
  # Place theta exactly in the middle of the arc
  annotate("text", x = label_r * cos(mid_angle), y = label_r * sin(mid_angle), 
           label = "theta", parse = TRUE) +
  
  # Maintain coordinate ratio so the arc remains circular
  coord_equal() +
  theme_minimal() +
  labs(title = expression("Angle between y and " * hat(y)), x = "x", y = "y")

```

When $cos(\theta) = 1$, $\theta = 0 \to$ $y$ and $\hat{y}$ are the identical
...
- when $y$ and $\hat{y}$ orthogonal, $R^2$ is $0
- when $y$ and $\hat{y}$ approach each other, $R^2 \to 1$
- "how well does $\hat{y}$ approximate the *direction* of $y$"
- not **distance** between $y$ and $\hat{y}$; scale-less: though distance is $0$ when $\theta = 0$
...
These $R^2_{Y \sim U | D,X}$ and $R^2_{D\sim U | X}$ (or "partial" $R^2$) can correspond to any $Y \sim f(U)$: how well any function of $U$ "predicts" $Y$/$D$

- but now we choose **cosine similarity**: same angle, not distance
- Less restrictive than simple OVB formula
- agnostic about functional form/interactions in the confounding
...
One major limitation with this approach is... what counts as "a lot" of residual variance explained? What counts as "a little"? WTF is $cos^2(\theta)$?

>- To make these easier to interpret, we want an interpretable benchmark. 
...
- Effectively: **consider all plausible DAGs** given a set of possible confounders
...
>- sometimes that is a lot of combinations
...
- histogram of coefficient estimates (not fully informative)
- histogram of $p$ values (can be misleading)
- histogram of $t$ statistics (better)
- specification curve (my favorite)
...
2. "Rug plots" indicating each attribute of a model, with lines
