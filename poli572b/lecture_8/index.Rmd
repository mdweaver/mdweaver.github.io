...
```{r, echo = F}
father.son$r = m$residuals
ggplot(father.son, aes(x = fheight, y = r)) + geom_point(alpha = 0.5) + 
    xlab("Father's Height (in)") + ylab("Residual Son's Height (in)") + labs(title = "Residual Son's height by Father's Height")   + geom_smooth(method = 'lm', se = F, colour = 'red', fullrange = T) + theme_bw()
```
...
require(splines)
ggplot(plot_data, aes(x = AGE, y= UHRSWORK)) + geom_point(alpha = 0.01) + geom_smooth(method = 'lm', colour = 'red', se = F) +  geom_smooth(method = 'lm', formula = y ~ splines::ns(x, 30), colour = 'blue', se = F) + theme_bw() + facet_wrap(~type) + ggtitle("Residual Hours Worked on Age")
```
...
```{r, echo = F, warning=F, message=F}
ggplot(plot_data, aes(x = UHRSWORK, y = INCEARN)) + geom_point(alpha = 0.01) + geom_smooth(method = 'lm', colour = 'red', se = F) + theme_bw() + facet_wrap(~type)  + ggtitle("Earnings on Residual Hours")
```
