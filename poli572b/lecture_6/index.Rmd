...
>- we need to "learn" this function from the data (without "overfitting")?
...
>- This naturally connects to the "imputation" approach to conditioning

---

### Conditional Expectation Function

We make **choices**/**constraints** to make it possible to **estimate** the CEF:
...
- We need mean of $Y$ across values of $X$ $\to$ LS is a generalization of the mean
- (comparatively) "Simple", yet flexible tool to generate predictions/plug in missing counter-factuals
- Ubiquitous, often "good enough"

<br>

>- As we move from design-based to model-based inference, we need to be in control of the modelling tool we use.
...
In order to use least squares for causal inference, need to understand what the **math** of regression is doing:

- given need for averages: how is regression related to the mean?
- given need to "impute": how does regression "plug-in" missing counterfactuals?
...
>- For random variables, it is "best guess" of what value we will get on a given draw
...
What is the distance between these two points? $(3,0)$ and $(0,4)$
...
One way of thinking about the mean is as a **prediction** algorithm. 

We observe $n$ values of $Y$: $y_1 \dots y_n$. This has a coordinate in $n$-dimensional space.

We want to predict $Y$ with $\hat{y}$ (also a coordinate in $n$ dimensional space), with two constraints:

1. $\hat{y}$ has the same value in each dimension... (lives at point $(\hat{y} \dots \hat{y})$)
2. Our criterion for "best" prediction is having the **shortest distance** between $y_1 \dots y_n$ and $\hat{y}$ is is

If we impose this rules, we end up at choosing the **mean** as our **prediction algorithm**

---

### The Mean

This "Geometric Interpretation" is helpful:

- since distances are sums of squared differences, this is why mean minimizes the **variance** of prediction errors ($(y_i - \hat{y})^2$).
- key to understanding how least squares is connected to the mean
- key to understanding how least squares works (how does it impute? how does it "control" for variables?)
...
The span of any vector always goes through the **origin** $(0,0)$: Why?
...
<iframe width="560" height="315" src="https://www.youtube.com/embed/kYB8IZa5AuE?si=eqeg42cLrhUdouL-&amp;start=364" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
...
>- matrix multiplication, covariances
...
and another vector $\mathbf{e}$, which is difference between the prediction vector and the vector of observations (**residuals**):
...
- we don't **need** geometric intuition for this

---

### Bivariate Regression 

If we choose slope $b_1$ and intercept $b_0$ such that they minimize prediction errors $\sum_{i=1}^{n} (y_i - \hat{y}_i)^2$, we can obtain these formulae:
...
But to go beyond bivariate regression, need a more general solution:
