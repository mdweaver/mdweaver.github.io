...
- But we have not considered whether we could see observed effects **by chance** (probability of finding this evidence, when there is no effect)
...
What are the other (**counterfactual**) outcomes that we could have observed through implementing this procedure/test **due to random process**?


---

Severe testing in political science $\to$

1. **unbiased**  causal effect estimates

2. correctly describing **sampling distributions** 
...
- what is probability of observing this estimate, using this process, assuming some true value of the parameter/estimand. (or what is probability that we draw conclusion **in error**)
...
We can only estimate and use these estimated sampling distributions...
...
- aim of statistics is to make inferences about unknown quantities. 

if the unknown is about a **population** $\to$  a descriptive inference

- $\to$ randomness in which cases in the population are sampled

if the unknown is about **potential outcomes** $\to$  a causal inference

- $\to$ randomness in which potential outcomes are observed for cases
...
At what **level of analysis** does random selection occur? (at what level are we taking **independent** draws?) $\to$ different sampling distributions

- do we sample **individuals** or **groups of individuals**? Are individuals randomized into treatment? Or are villages (with many individuals together) randomized into treatment?
...
- There are analytic estimators (equations); bootstrap methods; randomization inference; etc..
- All variance/sampling distribution estimators involve specific assumptions to "work"
- **and** "working well" depends on the attributes of the data and context (e.g. sample size, number of clusters, degree of spatial correlation, etc.)
...
>- Where does the randomness making the sampling distribution come from?
...
Some randomness comes from a **random process** of realizing some **potential outcomes** versus other potential outcomes. As the sample approaches the full population, this variability does not approach 0 (though it gets smaller).
...
we have one realization of the data generation process. We attempt to estimate the sampling distribution.
...
<img src="https://www.denardi.ca/media/1_25180.jpg">
...
- this estimator is not **unbiased** but **consistent** as $n \to \infty$
- If $n$ is too small, $ehw$ SEs can be too small (see Angrist and Pischke section 8.1)
...
<img src="https://images.cdn.saveonfoods.com/detail/00287990000009.jpg">
...
<img src="https://tasteofartisan.com/wp-content/uploads/2020/05/Salame-Toscano-Tuscan-Salami.jpg">
...
- Conley-Kelly 2025 introduce spatial placebos and spatial clustering  [R package](https://morganwkelly.github.io/spatInfer_tutor/tutorial.html)
