...
What is the estimate, $\widehat{ACE}$?, comparing treated to untreated?
...
- We can either: examine relationship between $D$ and $Y$, within groups of cases where values of $\mathbf{X_i}$ are the same (hold value of backdoor paths constant), adjusting for probability of $D$ for given values of $\mathbf{X_i}$
...
Condition on what to find the effect of ability on earnings (if we can't directly observe ability)?
...
- May suffer from motivated reasoning
...
Instead, we find effect of $D$ on $Y$ within each subset of the data uniquely defined by values of $X_i$. 
...
If observed values of $Y(1)$ are systematically different than $E[Y(1)]$  (bias), that must be because some units are more/less likely to have $D=1$.
