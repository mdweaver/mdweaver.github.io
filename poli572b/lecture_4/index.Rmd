...
How does this compare against assumptions made when analyzing experiments?
...
What is the estimate, $\widehat{ACE}$?, if we just compare mean racial resentment of TFA participants to non-participants?
...
Because people select themselves into TFA, no randomization... if we reran this process again we would get the same result.  **No alternate** allocations of TFA enrollment possible.
...
**Confounding** occurs when, in the cases we observe, the values of treatment $D$ are **not independent** of potential outcomes of $Y$. Lead estimator $\widehat{ACE}$ to be **biased**
...
- **Occurs usually when we don't have randomization**
...
Start with our "plug in" estimator of $ACE$:

$$E[\widehat{ACE}] = \overbrace{E[Y_i(1) | D_i = 1 ]}^{\text{mean Y(1) in treated}} - \underbrace{E[Y_i(0) | D_i = 0]}_{\text{mean Y(0) in untreated}}$$

Add and subtract counterfactual $\color{red}{E[Y_i(0) | D_i = 1]}$

$$
\begin{align*}
E[\widehat{ACE}] =& E[Y_i(1) | D_i = 1 ] - \color{red}{E[Y_i(0) | D_i = 1]}  \\
&+\color{red}{E[Y_i(0) | D_i = 1]} - E[Y_i(0) | D_i = 0]
\end{align*}
$$
...
$$
\begin{align*}
E[\widehat{ACE}] =& \overbrace{E[Y_i(1) | D_i = 1 ] - \color{red}{E[Y_i(0) | D_i = 1]}}^{\text{ACE for treated (ATT)}}  \\
&+ \underbrace{\color{red}{E[Y_i(0) | D_i = 1]} - E[Y_i(0) | D_i = 0]}_{\text{treated-untreated diff. in Y(0)}}
\end{align*}
$$
If randomization... Is the top term different from ATU/ATC? What is the bottom term?

We need find substitution for $ATT$ in terms of $ACE$...
...
$$ACE = \pi ATT + (1 - \pi) ATU$$
$$ACE = \pi ATT + (1-\pi) ATT - (1-\pi)ATT + (1 - \pi) ATU$$
$$ACE = [\pi + (1-\pi)] ATT - (1-\pi)ATT + (1 - \pi) ATU$$
$$ACE = ATT - (1-\pi)(ATT - ATU)$$
$$ACE + (1-\pi)(ATT - ATU) = ATT$$
...
$$\begin{align*}
E[\widehat{ACE}] =& \overbrace{E[Y_i(1)] - E[Y_i(0)]}^{\text{ACE}} \ + \\
& \overbrace{(1-\pi)(ATT - ATU)}^{\text{Heterogeneity Bias}} + \\
& \overbrace{\color{red}{E[Y_i(0) | D_i = 1]} - E[Y_i(0) | D_i = 0]}^{\text{Selection Bias}} 
\end{align*}$$
...
**selection bias**: *baseline* differences in $Y(0)$ between "treated" and "untreated" units ($D=1$, $D=0$). Treated/untreated groups **already** different in absence of treatment.

**heterogeneity bias**: **effects** of treatment differ between "treated" and "untreated" units.

---

### Confounding

**Easier to understand confounding graphically**

- what it is, how it arises
- doesn't distinguish between SOURCES of bias (selection/heterogeneity)
...
tidy_dagitty() %>%
  ...
  geom_dag_text(aes(label = label), color = "black", alpha = 0.8) +
geom_dag_edges() +
theme_dag()
```
...
>- Other solutions to confounding?
...
2. "Block" backdoor paths by "holding variables constant": **conditioning**
...
- Where $\mathbf{X_i}$ is a set of variables (we suspect) appear on backdoor paths from $D$ to $Y$.
- We use values of $\mathbf{X_i}$ to:
    - **impute** missing counterfactual values of $Y_i$ across values of $D_i$ (within strata) where values of $\mathbf{X_i}$ are the same
...
Want to **Condition on** variables that
...
**colliders**: variables that are causally influenced by two or more variables (2+ arrows pointing into it). The causal variables influencing the collider are themselves not necessarily associated.
...
**mediators**: variables that are on the path from $D$ to $Y$ ("how" $D$ affects $Y$)

- **conditioning** on a **mediator** $M$: possibly a collider (other things affect $M$); "blocks" path from $D$ to $Y$
...
- Can help us be **explicit** about our assumptions (what backdoor paths or colliders can/cannot exist)
...
- **imputation**/**matching**: "plug in" unobserved potential outcomes of $\color{red}{Y_i(1)}$ ($\color{red}{Y_i(0)}$) using observed potential outcomes of $Y_j(1)$($Y_j(0)$) from cases with **same**/**similar** values of $\mathbf{X_i} \cong \mathbf{X_j}$.
...
Then take a **weighted average** of all $\widehat{ACE}[X_i = x]$, weighted by $Pr(X_i = x)$ (fraction of cases where $X_i = x$)
...
But we lack random assignment; possible selection bias ($Y_i(0)$ already different for joiners and heterogeneity bias (effect of $TFA$ different for joiners).
...
3. Machine Learning: (impute predicted values of $Y$ using statistical learners)
...
If we knew **the probability each case was treated** ($Pr(D_i)$) then we can **re-weight** observed values of $Y(1)$ such that we **reconstruct** the $E[Y(1)]$. The same can be done for $Y(0)$
...
- holding $X = x$, like assigning treatment at random, with probability $p$
...
```{r, echo = F}
po.table = data.table(i = 1:8, 
                      y_i_0 = c(3, 3, 3, 3, 2, 2, 2, 2),
                      y_i_1 = c(1, 1, 1, 1, 1, 1, 1, 1),
                      d_i = c(0,0,0,1,1,1,1,0),
                      x_i = c(1,1,1,1,0,0,0,0)
                      )
...
1. Estimate Propensity Scores: known probabilities, logit regression, machine learning, balancing weights
- different ways to get treatment probabilities (assuming conditional independence)
2. Estimate $ACE$ with inverse probability weighting
- different choices of weight calculations
