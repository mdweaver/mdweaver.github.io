...
How do we get unbiased estimate of $ACE$ or other causal parameters?
...
>- Why might value of treatment (TFA)  **not  be independent** of potential outcomes of $Y$?
...
- any path that remains after we remove arrows point out from $D$
...
$\widehat{ACE} = E[Y(1) | TFA = 1] - E[Y(0)|TFA=0]$

which we found to be biased... $ACE \neq E[\widehat{ACE}]$

---

Instead, we find effect of $TFA$ on $Y$ within each subset of the data uniquely defined by values of $X_i$. 

$\widehat{ACE}[X = x] = \overline{Y(1)}[X = x] - \overline{Y(0)}[X = x]$ for each value of $x$ in the data. Where $\overline{Y}$ is the mean of observed values of $Y$ for 
...
1. **Ignorability**/**Conditional Independence**: 
    ...
    - can argue this is plausible using theory/knowledge of  cases
