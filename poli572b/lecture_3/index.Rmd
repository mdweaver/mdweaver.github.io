...
- people assigned to get off social media; some get on anyway
...
unless there is perfect compliance ($D_i = Z_i$ for all units $i$).
...
In policy-oriented contexts, $ITT$ is usually desirable. Always good to report $ITT$ ("**reduced form**" in instrumental variables)
...
If we add to the $ACE$ assumptions, we can find the $ACE$ for **compliers**: the **complier average causal effect** ($CACE$. **estimand**
...
To understand how, we first need a new potential outcomes model.
...
- Assume that "defiers" do not exist.
    - Seems plausible, but sometimes defiers may exist. (example: school birth cut offs)
...
Mean  of $x$, $\bar{x}$ can always be decomposed as a **weighted average** of sub-group means of $x$, $\bar{x}_g$, for sub-groups $g$:
...
Doing some rearranging, we just need to estimate two **estimands**/**parameters** to get the $CACE$:

$$\frac{ITT}{\pi_c} =  CACE $$

We need to estimate $ITT$ and $\pi_c$
...
We can estimate $\pi_c$ without bias: just use estimator $\widehat{ACE}$ with outcome as $D$ instead of $Y$
...
as $n \to \infty$, the bias goes to $0$. Thus, this approach is biased in **small samples** (typically, need several hundred observations or more)
...
What does the $CACE$ tell us?: It is the **effect of treatment** for the units that **change treatment status** due to **random assignment**

- **external validity**?: are the cases that comply with treatment assignment different from population of interest? Often, yes. CACE is limited only to compliers and may be uninformative about others. (**are compliers weird?**)

- **"weak instruments"**?: does random assignment actually induce a change in "taking treatment"? If intervention doesn't affect behavior ($ITT_D$ is small), these approaches prone to bias, incorrect estimates of standard errors. (beyond weird compliers, **estimator has bad properties** when "nudge" to treatment is too weak.)
...
- if treatment assignment/treatment takes multiple values, assumptions/interpretations change.
- If someone has "**instrumental variable**" with an experiment or **natural experiment**, can you map it onto this framework? If not, be suspicious. (examples?)

- Assumptions here are "bigger":
    - "no defiers"/monotonicity not always plausible (partial identification available)
...
5. Interpret the results from the estimates $\widehat{CACE}$ (What do they mean, substantively? Might they be biased?)
