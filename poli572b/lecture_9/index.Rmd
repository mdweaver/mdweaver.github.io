...
1. Regression and Conditioning

2. How regression "conditions"
...
### Regression for Conditioning

- How does regression relate to our causal **estimands**?
- Curse of dimensionality $\to$ regression:

    - how does regression impute?
    - what challenges arise with conditional independence in LS?
    - how is common support an issue in LS?
...
<!-- If we don't know parameters $\beta_0, \beta_1$, what do we need to assume to obtain an estimate $\widehat{\beta}_1$ that we can give a **causal** interpretation? -->

<!-- **We must assume** -->

<!-- - $Y_i$ actually produced according to the response schedule (equation is correctly specified) -->
<!-- - $D_i$ is independent of $\epsilon_i$:  $D_i \perp \!\!\! \perp \epsilon_i$. (equivalent to randomly assigned) -->
...
>- What are the weights in regression?

---

### Regression: Causal Effect Weights

[Aronow and Samii (2016)](https://doi.org/10.1111/ajps.12185) discuss how regression weights cases, with a binary treatment.
...
Where $w_i = [D_i - E(D_i | X_i)]^2$: the square of $D$ residual on $\hat{D}$ predicted by $X$.
...
- in the best case scenario where $D$ is truly linear in $X$, these weights make sense (more weight when more variation in $D$, conditioning on $X$)
...
- we need to ask, for **what set of cases** are we estimating effect of $D$?
...
<img src="./angrist_krueger.png" width = 55%>

---

<img src="./angrist_pischke.jpeg" width = 80%>
...
- weights only easily interpreted under specific conditions
- more weights on values of $d$ near the median; less well predicted by $X$
- consider also: common support... are **all values** of $d$ present at near all values of $X$?
...
>- To see this, need to see how regression imputes.
...
1. Regress `success` on `untype4` ($D$) and `logcost`, `wardur`, `factnum`, `trnsfcap`, `treaty'`, `develop`, `exp`, `decade`, using `lm()`, save this as `m`
2. Use model to predict *counterfactual* potential outcomes for each conflict:
  - Create a copy of `ds2000` called `ds2000_cf`, and flip the value of `untype4` (0 to 1, 1 to 0).
...
- Rather than **hold confounders constant**, we obtain "plug-in" equation coefficients requiring $X$ and $D$ to be orthogonal (perfectly $0$ linear correlation).
...
- Are the earnings ($Y$) related to gender and age ($X$) in an **additive** way? (relationship between age and earnings the same for men as for women?)
...
ggplot(plot_data, aes(x = AGE, y = r_hours_3 %>% round, colour = Gender, group = Gender, weight = n, size = n) ) + geom_point() + geom_smooth() + xlab("Age (years)") + ylab("Mean Residual Hours Worked") + geom_hline(yintercept = 0) + ggtitle("Residual Hours Worked (Assuming neither Linearity nor Additivity)") + theme_bw()
...
(1) Interpolation bias: model dependence $\to$ failure of conditional independence, even if backdoor paths "blocked"

(2) Extrapolation bias: model dependence + lack of common support $\to$ bias
...
ate_hat = ds2000[, ifelse(untype4 %in% 1, success - y_hat, y_hat - success)] %>% mean %>% round(3)
...
In the linear context, the estimated average treatment effect of peacekeeping is `r ate_hat`
...
ate_hat_alt = ds2000[, ifelse(untype4 %in% 1, success - y_hat, y_hat - success)] %>% mean %>% round(3)

```

Then the estimated average treatment effect is: `r ate_hat_alt`
...
### What to do about... Interpolation bias:

- flexible functional forms... polynomials not a great choice (show slide from last week)
  
- natural splines:

    - splines: fit different slopes for different segments of the data...
    - basics: cubic polynomial that are "piece-wise", but must be continuous at cutpoints; linear at ends
    - example... hours worked
    - choice of knots...
        - how do we choose? cross validation
        - cross-validation explained

---


### What to do about... Extrapolation bias:
...
**Splines** [https://bookdown.org/ssjackson300/Machine-Learning-Lecture-Notes/splines.html](See here) 
...
```{r, echo = F, warning=F, message=F}
fit1 = predict(lm(y ~ x), newdata = list(x = sort.x))

plot(x, y, cex.lab = 1.1, col="darkgrey", xlab = x.lab, ylab = y.lab,
     main = "", bty = 'l')
lines(sort.x, fit1, lwd = 2, col = "black")
```
...
cuts = quantile(x, probs = c(1:3)/4)

fit2 = predict(lm(y ~ bs(x, degree = 1, knots = cuts)), newdata = list(x = sort.x))

plot(x, y, cex.lab = 1.1, col="darkgrey", xlab = x.lab, ylab = y.lab,
     main = "", bty = 'l')
lines(sort.x, fit1, lwd = 2, col = "black")
lines(sort.x, fit2, lwd = 2, col = "red")
...
fit3 = predict(lm(y ~ bs(x, degree = 3, knots = cuts)), newdata = list(x = sort.x))

plot(x, y, cex.lab = 1.1, col="darkgrey", xlab = x.lab, ylab = y.lab,
     main = "", bty = 'l')
lines(sort.x, fit1, lwd = 2, col = "black")
lines(sort.x, fit2, lwd = 2, col = "red")
lines(sort.x, fit3, lwd = 2, col = "blue")
...
# clean and combine
  out <-  bind_rows(models %>% modsum_fun)
