...
- When we propose theory to explain some phenomenon: have we considered rival explanations?
...
## Do Social Media Algorithms Drive Political Polarization?

Everyone says yes, but how do we know?
...
- how does this relate to social media and polarization?

## Social Media Drives Polarization?

#### In pairs: 

What do the authors do to subject claim that social media algorithms drive polarization to severe testing? (What specific role do **statistical** tests play in assessing whether social media algorithms **cause** polarization?)
...
**What does it mean to say that social media feed algorithms *cause* political polarization?**
...
Imagine you were a participant in an experiment similar to the Piccardi et al paper...

Imagine that, today, you were asked whether you'd have lunch with a supporter of a rival political party (0 being no, 1 being yes). 

Write down: what would be your feeling (0 being no, 1 being yes)...

- ... if, yesterday, you were in the **no change**  group 

- ... if, yesterday, you were in the **downranking** group

## Potential Outcomes

In an experiment with two conditions: each subject, prior to intervention has two possible states that it could take.

If $Y_i$ is the willingness to have lunch with person $i$; $Z_i$ is the experimental condition: $1$ indicates downranking, $0$ no change, then we have two **potential outcomes**

1. lunch status with **no change** is $Y_i(0)$
2. lunch status **with downranking** is $Y_i(1)$
...
Using information about the treatment $Z$ (downranking or no downranking) and the outcome $Y$ (lunch or no lunch), we can infer a **range** of values for each unit treatment effect:
...
More complex experiments (e.g. testing "spillover effects") imply different estimands, different estimators.
