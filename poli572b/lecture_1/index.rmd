...
*What kinds of assumptions are "baked in" to this response schedule?*
...
The potential outcomes model points towards possible solutions:
...
- different approaches we can take to solve this problem
    ...
    - another approach makes more assumptions, but gives us a *point estimate* (point identification)

## Potential Outcomes

The potential outcomes model points also points toward different causal **estimands**:

- We might want **unit causal effects**
- We might want the **average causal effect**
- We might want the **average treatment effect on treated**
- different effects may be of interest, depending on the question

# Partial Identification

## Partial Identification

**identification**: when an attribute of an unobserved/population distribution can be inferred without bias to have a specific value, given the observed data.
...
Returning to our hypothetical experiment, what **possible** values could each unobserved potential outcome take?
...
$$\begin{equation}\begin{split}E[\tau_i] &= \{E[Y_i(1)|Z_i = 1]Pr(Z_i=1) + E[Y_i(1)|Z_i = 0]Pr(Z_i=0)\} \\ & \phantom{=}\ - \{E[Y_i(0)|Z_i = 1]Pr(Z_i=1) + E[Y_i(0)|Z_i = 0]Pr(Z_i=0)\} \end{split} \end{equation}$$

We decompose the mean of $Y(1)$ and $Y(0)$ as a weighted average by principal strata: those treated ($Z=1$) and untreated ($Z=0$)

## Partial Identification

Some of the means within strata are $\color{black}{observed}$ and other are $\color{red}{counterfactual}$:

$$\begin{equation}\begin{split}E[\tau_i] &= \{E[Y_i(1)|Z_i = 1]Pr(Z_i=1) + \color{red}{E[Y_i(1)|Z_i = 0]}Pr(Z_i=0)\} \\ & \phantom{=}\ - \{\color{red}{E[Y_i(0)|Z_i = 1]}Pr(Z_i=1) + E[Y_i(0)|Z_i = 0]Pr(Z_i=0)\} \end{split} \end{equation}$$
We can then plug in the maximum and minimum possible values for the unobserved counterfactuals, to calculate the highest and lowest possible values $E[\tau_i]$

## Partial Identification

If $Y \in {Y^L, Y^U}$ (the minimum and maximum possible values of $Y$ are $Y^L, Y^U$ respectively):

$$\begin{equation}\begin{split}E[\tau_i]^U &= \{E[Y_i(1)|Z_i = 1]Pr(Z_i=1) + \color{red}{Y^U}Pr(Z_i=0)\} \\ & \phantom{=}\ - \{\color{red}{Y^L}Pr(Z_i=1) + E[Y_i(0)|Z_i = 0]Pr(Z_i=0)\} \end{split} \end{equation}$$
...
In a sense, there are no "statistics" here, as we don't appeal to any random variables.

We can algebraically express bounds implied by specific assumptions.
