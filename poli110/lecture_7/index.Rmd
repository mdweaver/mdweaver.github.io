...
### (2) How to Evaluate Descriptive Claims
...
In small groups: what do we need to do so we can evaluate this claim? (What steps must we take?)

> **"Democracies have been backsliding (becoming less democratic) since the election of Donald Trump"**

## Descriptive Claims: Evaluation

### **Concepts**: 

define our terms in a way that is **transparent** and can be used **systematically**

### **Variables**:

translate concepts into something that we can (in principle) **observe**

### **Measurement**:

devise **transparent** and **systematic** procedures with known **uncertainty** to observe those attributes of specific cases.  
...
clear and accessible definition. Even if we disagree over the label, we can all use the concept.
...
are **abstract** or **general** categories that we apply to **particular** cases using with a set of rules/criteria that determine membership in the category. The rules/criteria that determine membership are:
...
- observable: defining traits in a concept **must be something we can all observe** (empirical). (e.g., democracy cannot be defined by an ethereal 'Democratic Spirit')
- relevant: traits are **relevant to predicting** how cases belonging to the concept affect other things, are affected by other events, or are part of a causal process.
...
1. **Kim Jung Un's: "a state with 'democratic' in its name."**
...
Depending on the kinds of questions we are interested in, we may have different concepts, different definitions for the same concepts. 

**Even if we disagree with <u>definitions</u> for specific word**, good scientific concepts can be understood and used by others regardless of whether they agree with our definitions.
...
4. **Scientific concepts are "objective" in the sense they can be used even if we disagree with them**
  - "subjective" in sense that value judgements tell us one definition is better or worse.
