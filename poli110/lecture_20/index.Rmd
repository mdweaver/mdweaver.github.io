...
Does exposure to mass-shootings increase support for laws restricting access to guns? ([McGinty et al 2013](https://psychiatryonline.org/doi/10.1176/appi.ajp.2013.13010014))
...
<img src="gun_control_experiment.jpg" width = 50%>
...
ggplot(data, aes(x = log_pop, y = value1, color = Candidate )) + geom_jitter(height = 0.05, alpha = 0.1) +  geom_smooth(method = 'gam', se = FALSE) + 
  ylab("(X) Rallies (Any)") +
  xlab("(W) Log population") +
  ggtitle("Population (W) and Rallies (X) by County (2016):\nCorrelation") + 
  theme_bw()
```
...
ggplot(data, aes(x = log_pop, y = value2, color = Candidate )) + geom_jitter(height = 0.05, alpha = 0.1) +  geom_smooth(method = 'gam', se = FALSE) + 
  ylab("(Y) Hate Crimes (per Month)") +
  xlab("(W) Log population") +
  ggtitle("Population (W) and Hate Crimes (Y) by County (2016):\nCorrelation") + 
  theme_bw()
```
...
|--------------------|------------------------------|------------------------------------------------|---------|----------------------|----------------------|
| Experiment | Randomization<br>Breaks $W \rightarrow X$ link | **All** confounding variables | $X$ is random;<br> Change only $X$ | High | Low | 
| Conditioning | Hold confounders<br>constant | Only variables <br> conditioned on | Condition on all confounders | Low | High |
