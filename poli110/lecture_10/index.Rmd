...
#Get group means
resp = GET("https://api.mentimeter.com/questions/dca96accd7d7/result")
json =content(resp, "parsed")
g_means = json$results %>% lapply(function(i) get_data(i)) %>% lapply(function(x) mean(x[x>1]))
points = data.table(g = 1:5, mean = unlist(g_means))

#json$results %>% lapply(function(i) get_data(i))

#Sample:
#sample
n = floor(length(c_values[c_values > 1])/5)
#sample vec
s = rep(NA, 10000)

#loop samples
for (i in seq_along(s)) {
  s[i] = sample(c_values[c_values > 1], size = n) %>% mean
}

ggplot() + geom_histogram(aes(x = s), col = "white", bins = 50) + theme_bw() + geom_vline(xintercept = points$mean, col = 'red') + geom_vline(xintercept = mean(c_values[c_values > 1]), col = 'blue', linetype = 2) + xlab("Commute Times of Sample") + ylab("Count")

```

Avg. commute time for 10000 random samples from the survey on page 33 of the slides. Blue line is population avg. Red lines are the sample avgs. from the 5 phone number groups.
