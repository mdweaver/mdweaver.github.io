...
g_cut = data.table(start = (cutpoints %>% head(-1))*max_pts,
                   end = (cutpoints %>% tail(-1)) * max_pts,
                   grade = factor(c("F", "D", "C-", "C", "C+", "B-", "B", "B+", "A-", "A", "A+"), levels = c("F", "D", "C-", "C", "C+", "B-", "B", "B+", "A-", "A", "A+"))
)
g = fread("./midterm_grades.csv")

g[, grade := cut(midterm, max_pts*cutpoints, right = F, labels = c("F", "D", "C-", "C", "C+", "B-", "B", "B+", "A-", "A", "A+"), include.lowest = T)]

ggplot(g, aes(x = midterm, fill = grade)) + 
  geom_histogram(data =g, aes(x = midterm, fill = grade)) +
...
ggplot(g[!is.na(grade)], aes(x = grade, fill = grade)) + 
      geom_bar(aes(y = (after_stat(count))/sum(after_stat(count)))) +
  theme_bw() +    scale_fill_viridis_d() +
    scale_y_continuous(labels = scales::percent) +
  ggtitle("Midterm Grade Distribution") + 
  xlab("Grade") + ylab("% of Class")
```
...
```{r }
g_old = fread("./midterm_grades_2024.csv")

ggplot(g[!is.na(midterm)], aes(x = midterm)) + 
  geom_rect(data = g_cut, aes(xmin = start , xmax = end, ymin = -Inf, ymax = Inf, fill = grade),
            inherit.aes=FALSE, alpha = 0.3) +
  geom_text(data = g_cut, aes(x = (start+end)/2, y = 0.95, label = grade), alpha = 0.3) +
  scale_fill_viridis_d() +
  geom_step(aes(y = 1 - after_stat(y)), stat='ecdf') + 
  geom_step(data = g_old[!is.na(midterm)], aes(x=midterm/25*30 , y =  1 - after_stat(y)), stat = 'ecdf', colour = 'darkgrey') +
  theme_bw() + 
  ggtitle("Midterm Grade Distribution") + 
  xlab("Score") + 
  ylab("Fraction with score > than X") +   scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.1))
```
...
>- Is there any reason to doubt that this **correlation** is the true **causal effect** of mask mandates?
...
- this happens because some other factor $W$ affects $X$ and potential outcomes of $Y$
...
>- Correlation of gun ownership and firearms homicides for US states.
...
crime[, gun_ownership_rand := gun_ownership_rand*100]
crime = crime[gun_murder_percapita<10,]

ggplot(crime, aes(x = gun_ownership_rand, y = cdc_gun_homicides , label = State)) +
  geom_text() +
...
- if the confounding induced an **upward** bias (observed correlation between gun ownership and firearms deaths **more positive** than true causal relationship)?

---

```{r, echo = F, message=F, warning=F}

coefs = lm(cdc_gun_homicides ~ gun_ownership_rand, crime) %>% coefficients
ggplot(crime, aes(x = gun_ownership_rand, y = cdc_gun_homicides , label = State)) +
  geom_text() +
  xlab("Gun Owners (%)") +  ylab("Firearms Homicides Per Capita") + 
  ggtitle('What if true causal effect is the red line?\nAnd correlation is in blue?') + 
  geom_smooth(method = 'lm', se = F, fullrange = T) + 
  geom_abline(intercept = coefs[1], slope = coefs[2] - 0.05, color = 'red' ) +
  geom_segment(aes(x = 60, 
                   y = coefs[1] + (coefs[2] - 0.05)*60, 
                   xend = 60, 
                   yend = coefs[1] + (coefs[2])*60),
    arrow = arrow()
  ) +
  geom_abline(intercept = coefs[1], slope = coefs[2] - 0.1, color = 'red' ) +
  geom_segment(aes(x = 55, 
                   y = coefs[1] + (coefs[2] - 0.1)*55, 
                   xend = 55, 
                   yend = coefs[1] + (coefs[2])*55),
    arrow = arrow()
  ) +
  theme_bw()
```
...
- if the confounding induced a **downward** bias (observed correlation between gun ownership and firearms deaths **more negative** than true causal relationship)?
...
>- If bias is **upward** and correlation is **positive**, true causal effect direction is **unknown**. $\to$ Unclear what policy to choose. (Fails weak severity --- confounding may lead to positive correlation even if truth is negative causal effect)

>- If bias is **downward** and correlation is **positive**, true causal effect direction is **positive**. $\to$ Definitely impose gun restrictions. (confounding makes it **harder** to find this effect: strong severity) 
...
1. If confounding induces **bias** in **opposite direction of the correlation** $\to$ true causal effect in same direction as correlation (no failure of weak severity)


2. If confounding induces **bias** in **same direction as the correlation** $\to$ true causal effect may be zero or opposite direction of correlation (possible failure of weak severity)
...
- Can you think of any comparisons we can use that  *guarantee* no other variable $W$ affects $X$ (gun ownership) and $Y$ (gun violence)? 
...
4. Causal graphs help us diagnose the expected **direction** of bias (does confounding $\to$ evidence failing weak severity?)
