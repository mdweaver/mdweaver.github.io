...
>- Confounding arises because we use case B as a stand-in for counterfactual of case A, but case B is not actually like the counterfactual case B (in ways that affect $Y$)
...
Imagine you want to know if increasing per-student funding for a food security programs reduces food insecurity on university campuses. To answer this question, you look at the correlation between per-student funding for food security programs and the fraction of students experiencing food insecurity using **annual data** for all Canadian universities between 2018 and 2023.

Propose a causal graph that would imply that the correlation between funding for food security programs  and student food insecurity is biased due to confounding.
...
- Confounding can be diagnosed visually 
