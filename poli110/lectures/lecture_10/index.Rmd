...
<!-- ### How do we get *internal validity* -->

<!-- - We can get external validity on important questions -->
<!--     - with comparative method -->
<!--     - correlation -->
<!-- - how can we get closer to **internal validity** of experiments? -->
<!--     - how can we get empirical evidence on counterfactual? -->
>>>>>>> master
...
2. We can assess **probability** of chance correlation using:
...
<img src="./chocolate_1.png" width=80%>
...
- Is the result of a third or additional variables that affect $X$ and $Y$. This is called **confounding**.
...
Increased political support for secular parties reduces religious violence

### Causal theory:

Increases in the **fraction of votes won** by the Indian National Congress reduce the **frequency** of Hindu-Muslim **riots**.
...
text(.6, 5, paste0("P-value: ", sprintf("%.5f",round(c$p.value, 5))), pos = 4)
...
Why doesn't $X \rightarrow Y$ imply **perfect correlation**?
    - If $X$ causes $Y$, shouldn't we see nearly perfect relationship?
    
### Recall...

**probabilistic causal claims**: acknowledges **causal complexity**

- other causes affect $Y$ (e.g. $W,Z$) besides $X$
- effects of $X$ on $Y$ might be **conditional**
...
dag <- dag.init(y.name = "", x.name = "", 
        covs = c(1,1,1),
        arcs = c(1,-1,2,-1,3,-1),
        symbols = c('Income Growth','Campaign Spending', 'War Casualties', 'Party Ideology' ,'Incumbent Vote (%)'))
invisible(dag.draw(dag, legend = F))
```

## Digression

Is the correlation spurious?
...
As long as other factors unrelated to $X$ (Income growth):
- correlation still implies **causality**.
- other factors affecting $Y$ only induce weaker correlation between $X$ and $Y$.
...
## Campaign Contact and Vote Choice
...
invisible(dag.draw(dag, legend = F))
text(dag$x[2], dag$y[1] + 0.05, "Cause?")
text(mean(dag$x[1:2]), mean(dag$y[1:2]), "No cause", pos = 2)
```
...
- $X$ and $Z$ cause $Y$, and $X$ often correlated with $Z$.


## Beware Third Variables?

Does this mean we must always be concerned about any **third** variable besides the **cause** $X$ and **outcome** $Y$?

## Beware Third Variables?

Not necessarily:

1. If $W$ unrelated to $X$ or $Y$, then no **confounding**/no **bias**
2. If $W$ is an **antecedent** variable, no **confounding**/no **bias**
3. If $W$ is an **intervening** variable, no **confounding**/no **bias**
...
text(dag$x[2], dag$y[1], "Causal?", pos = 1)
```
...
text(mean(dag$x[1:2]), mean(dag$y[1:2]), "Causes\nIncrease", pos =2)
text(mean(dag$x[2:3]), mean(dag$y[2:3]), "Causes\nDecrease", pos =4)
text(dag$x[2], dag$y[1], "(-) Correlation:\nCausal", pos = 1)
```
...
dag <- dag.init(y.name = "", x.name = "", 
        covs = c(1,1),
        arcs = c(0, 1, 1,-1,0,2,2,-1),
        symbols = c('Economic\nGrowth (X)','Job\nCreation','Economic\nGrievance','Civil\nWar (Y)'))
invisible(dag.draw(add.arc(dag, arc = c(1,4), type = 1), legend = F, noxy=1))
text(mean(dag$x[2:3]), dag$y[1], "Causal", pos = 1)
text(mean(dag$x[1:2]), mean(dag$y[1:2]), "Increase", pos =2)
text(mean(dag$x[c(1,3,3)]), mean(dag$y[c(1,3,3)]), "Decrease", pos = 3)
text(mean(dag$x[c(2,4,4)]), mean(dag$y[c(2,4,4)]), "Decrease", pos = 1)
text(mean(dag$x[3:4]), mean(dag$y[3:4]), "Increase", pos =4)
...
```{r, echo = F}
invisible(dag.draw(add.arc(dag, arc = c(2,1), type = 0), legend = F, noxy=1))
```
...
|  | Third Variable? | Bias? |
|--------------------------|-----------------|----------------------------|
| Spurious/<br>Confounding | Yes | Yes |
| Antecedent Variables | Yes | No<br>If only **one path** |
| Intervening Variables | Yes | No |
| Reverse Causality | No | Yes |


## Correlation: Bias vs Random

### Random association:

We observe correlation of $X$ and $Y$

- But pattern occurred by chance
- No real relationship between values of $X$ and $Y$

### Bias

We observe a correlation of $X$ and $Y$

- It is unlikely to have occurred by chance (statistically significant)
- But it **does not** result from causal link between $X$ and $Y$
    - Spurious/confounding: $W \rightarrow X$, $W \rightarrow Y$
