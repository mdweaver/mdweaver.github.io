...
(1) We should mandate vaccine boosters (**prescriptive claim**)

(2) We should impose lockdown and masking orders (**prescriptive claim**)
...
1. $(H_1, H_2, \ldots, H_k) \xrightarrow{imply} O_1$
...
If a **claim** $H_1$ is falsfiable, there must be observable implication $O_1$ that, if observed, would invalidate $H_1$. So...

1. $H_1 \xrightarrow{implies}  not \ O_1$
2. If we see $O_1$
3. Then, $not\ H_1$
...
Two different **approaches to evidence**. May apply **verification** to a **falsifiable** claim.
...
- claim/theory includes an untestable reason for why observable evidence rejects the theory (e.g., conspiracy, flying spaghetti monster)
